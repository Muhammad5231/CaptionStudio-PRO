...
export function buildVerificationEmail(data: VerificationEmailData): { html: string; text: string; subject: string } {
  const subject = 'Verify your CaptionStudio PRO account';
...
<body>
  <div class="container">
    ...
    <div class="content">
      ...
      <p>Thank you for signing up for CaptionStudio PRO! To activate your studio account and access the dashboard, please verify your email address by clicking the button below:</p>
      
      <div class="button-container">
        <a href="${data.verifyUrl}" class="btn" target="_blank" rel="noopener noreferrer">Verify Account</a>
      </div>

      <div class="warning">
        <strong>Notice:</strong> This verification link is valid for ${data.expiresInMinutes} minutes and can only be used once. If you did not create a CaptionStudio PRO account, you can safely disregard this message.
      </div>

      <p>If the button doesn't work, copy and paste this link into your browser:</p>
      <p class="break-url">${data.verifyUrl}</p>
    </div>
    <div class="footer">
      &copy; ${new Date().getFullYear()} CaptionStudio PRO. All rights reserved.<br>
      Enterprise AI Media and Video Caption Production Engine.
  ...
  const text = `
CaptionStudio PRO — Verify Your Account

Thank you for signing up for CaptionStudio PRO!

To activate your account and access your workspace, please open the following verification link:
${data.verifyUrl}
